...
def verify_md5(file_path, dest_md5_digit, hash_algorithm="md5"):
    ...
    return file_hash.lower() == dest_md5_digit.lower()


def download_file(url, local_filename, dest_md5_digit):
    need_download = True
    if os.path.exists(local_filename):
        if dest_md5_digit and verify_md5(local_filename, dest_md5_digit=dest_md5_digit):
            need_download = False
            logging.info(f"Local exist {local_filename} already, would not download it again.")
        else:
            logging.warning(f"Local exist {local_filename} already but it's not same with target, would download new one from web.")
    if need_download:
